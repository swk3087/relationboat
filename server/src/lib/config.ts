...
export const config = {
  ...
  serviceDomain: process.env.SERVICE_DOMAIN ?? 'relationboat.kro.kr',
  ...
  googleRedirectUriDev: process.env.GOOGLE_REDIRECT_URI_DEV ?? 'http://localhost:3000/api/v1/auth/google/callback',
  googleRedirectUriProd: process.env.GOOGLE_REDIRECT_URI_PROD ?? 'https://relationboat.kro.kr/api/v1/auth/google/callback',
  fileStorageRoot: path.resolve(root, process.env.FILE_STORAGE_ROOT ?? './uploads'),
  fontBaseUrl: process.env.FONT_BASE_URL ?? 'http://localhost:3000/uploads/fonts',
