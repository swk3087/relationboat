...
const loadPeople = async () => {
  ...
  if (q || category) {
    ...
    allPeople = await apiRequest(`/api/folders/${encodeURIComponent(currentFolderId)}/search/people?${query.toString()}`);
    filterLocally();
    return;
  }

  allPeople = await apiRequest(`/api/folders/${encodeURIComponent(currentFolderId)}/people`);
  filterLocally();
};
...
const init = async () => {
  ...
  createForm.addEventListener('submit', async (event) => {
    ...
    try {
      await apiRequest(`/api/folders/${encodeURIComponent(currentFolderId)}/people`, {
        method: 'POST',
        body: {
          name,
          phone: phone || null,
