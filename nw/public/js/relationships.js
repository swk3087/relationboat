...
const loadPeople = async () => {
  ...
  peopleInFolder = await apiRequest(`/api/v1/folders/${encodeURIComponent(currentFolderId)}/people`);
  optionFromPeople(fromSelect);
  optionFromPeople(toSelect);
  optionFromPeople(sourceSelect);
  optionFromPeople(targetSelect);
...
const loadRelationshipSearch = async () => {
  ...
  const result = await apiRequest(
    `/api/v1/folders/${encodeURIComponent(currentFolderId)}/relationships/search${query ? `?${query}` : ''}`,
  );
...
const init = async () => {
  ...
  createForm.addEventListener('submit', async (event) => {
    ...
    try {
      await apiRequest(`/api/v1/folders/${encodeURIComponent(currentFolderId)}/relationships`, {
        method: 'POST',
        body: {
          fromPersonId,
          toPersonId,
  ...
  pathwayForm.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const payload = await apiRequest(
        `/api/v1/folders/${encodeURIComponent(currentFolderId)}/pathways?${params.toString()}`,
      );
