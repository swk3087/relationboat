...
const ensureSyncOnly = async (settings) => {
  ...
  const updated = await apiRequest('/api/v1/settings', {
    method: 'PATCH',
...
const loadSettings = async () => {
  const settingsResponse = await apiRequest('/api/v1/settings');
  ...
  const fonts = await apiRequest('/api/v1/settings/fonts');
  renderFonts(fonts);
};
